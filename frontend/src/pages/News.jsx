...
function impactTag(value) {
  if (!value) return { label: 'Neutral', cls: 'text-slate-300 border-white/10' };
  if (value > 0) return { label: `+${value.toFixed(2)}`, cls: 'text-neon-mint border-neon-mint/40' };
  return { label: value.toFixed(2), cls: 'text-neon-pink border-neon-pink/40' };
}
...
export default function NewsPage() {
  ...
  return (
    <div className="space-y-3">
      <div>
        <h1 className="font-display text-2xl text-white">News desk</h1>
        <p className="text-sm text-slate-400">Headlines decay over time — watch your positions.</p>
      </div>
      <div className="space-y-3">
        ...
        {news.map((n) => (
          <Card key={n.id}>
            ...
            <p className="mt-2 text-sm text-slate-400">{n.body}</p>
            <div className="mt-3 flex flex-wrap gap-2">
              {n.effects?.map((e) => {
                ...
                return (
                  <span
                    key={e.cropId}
                    className={`rounded-full border px-3 py-1 text-xs ${tag.cls}`}
                  >
