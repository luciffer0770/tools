  ...
  return (
    <div className="lg:grid lg:grid-cols-2 lg:items-start lg:gap-6">
      <div className="space-y-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <select
            ...
            className="min-h-[44px] flex-1 rounded-xl border border-white/10 bg-white/5 px-3 text-sm outline-none ring-neon-mint/30 focus:ring-2 sm:max-w-xs"
          >
            {crops.map((c) => (
        ...
        <Card className={`${isDesktop ? 'h-[420px]' : 'h-[320px]'} p-2`}>
          <div className="flex items-center justify-between px-2 pb-2 text-sm">
            ...
            <p className="font-display text-xl text-neon-mint">${crop.currentPrice.toFixed(2)}</p>
          </div>
      ...
      <details className="group mt-4 lg:mt-0" open={isDesktop}>
        <summary className="mb-3 cursor-pointer list-none rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-slate-200 lg:hidden">
          <span className="font-medium text-white">Trade controls</span>
          <span className="float-right text-neon-mint group-open:rotate-180">▼</span>
        </summary>
        <div className="space-y-4 lg:space-y-0">
          <Card className="lg:sticky lg:top-24">
            ...
            <div className="mt-4 space-y-2 text-sm text-slate-300">
              ...
              <div className="flex items-center justify-between">
                <span>Held</span>
                <span className="text-white">{holdingQty} units</span>
              </div>
            </div>
            ...
            <div className="mt-2 flex gap-2">
              <input
                ...
                className="min-h-[48px] w-full rounded-xl border border-white/10 bg-white/5 px-3 text-base outline-none ring-neon-mint/30 focus:ring-2"
              />
              {side === 'sell' && (
